...
RUN_CONFIG = {
    ...
    "KFOLDS": KFOLDS,
    "DROP_OUT": DROP_OUT,
    "MODEL": "DenseNet121 with liver+spleen+difference channels + attenuation stats",
    "OPTIMIZER": "AdamW + ReduceLROnPlateau",
    "LOSS": "BCEWithLogitsLoss",
    "COMMENT": """Tuned after reviewing the recent DenseNet runs.
Main changes:
- switch from independently resized liver/spleen crops to a shared union crop so relative organ context is preserved
- use channels: full shared crop, liver-masked crop, spleen-masked crop
- move back to moderate regularization and moderate fine-tuning instead of the over-restricted third run
- latest tuning adds a mild positive-class weight and saves by a combined AUC+recall score
- this helps avoid checkpoints that look good on AUC but miss too many positive cases
Uses 3 input channels:
1) full shared abdomen crop around liver+spleen
2) liver-masked shared crop
3) spleen-masked shared crop
...
def square_crop_from_box(img, y1, y2, x1, x2, pad_ratio=0.12):
    box_h = y2 - y1 + 1
    box_w = x2 - x1 + 1
...
def square_crop_from_mask(img, mask, pad_ratio=0.12):
    ...
    y1, y2 = ys.min(), ys.max()
    x1, x2 = xs.min(), xs.max()
    return square_crop_from_box(img, y1, y2, x1, x2, pad_ratio=pad_ratio)


def square_crop_from_union(img, mask_a, mask_b, pad_ratio=0.16):
    union_mask = ((mask_a > 0) | (mask_b > 0)).astype(np.uint8)
...
class LiverSpleenPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        shared_liver = square_crop_from_union(image_norm * liver_mask, liver_mask, spleen_mask)
        shared_spleen = square_crop_from_union(image_norm * spleen_mask, liver_mask, spleen_mask)
        ...
        shared_liver = np.array(
            Image.fromarray((shared_liver * 255.0).astype(np.uint8)).resize(
                (IMG_SIZE, IMG_SIZE), resample=Image.BICUBIC
            )
        ).astype(np.float32) / 255.0

        shared_spleen = np.array(
            Image.fromarray((shared_spleen * 255.0).astype(np.uint8)).resize(
                (IMG_SIZE, IMG_SIZE), resample=Image.BICUBIC
            )
        ).astype(np.float32) / 255.0

        stacked = np.stack([shared_crop, shared_liver, shared_spleen], axis=-1)
        ...
        if self.debug and self.saved < self.debug_max:
            self._save_debug_panel(
                ...
                liver_roi=shared_liver,
                spleen_roi=shared_spleen,
                signed_diff=shared_crop,
...
class DenseNet121WithStats(nn.Module):
...
def train_one_fold(model, train_loader, val_loader, criterion, optimizer, scheduler, fold):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        model_score = val_metrics["auc"] + BEST_SCORE_RECALL_WEIGHT * val_metrics["recall"]
        ...
        if model_score > best_score:
            ...
            no_improve = 0
            print(
                f"  -> Fold {fold} best model saved "
                f"(score={best_score:.4f}, val_AUC={best_auc:.4f}, recall={val_metrics['recall']:.4f})"
            )
        else:
            no_improve += 1
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(rows, labels_all), 1):
        ...
        if USE_POS_WEIGHT and len(class_counts) == 2 and class_counts[1] > 0:
            effective_pos_weight = min(class_counts[0] / class_counts[1], MAX_POS_WEIGHT)
            pos_weight = torch.tensor(effective_pos_weight, dtype=torch.float32).to(DEVICE)
        ...
        model = DenseNet121WithStats().to(DEVICE)
