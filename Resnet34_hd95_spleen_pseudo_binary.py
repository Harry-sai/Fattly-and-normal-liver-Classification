...
RESULTS_DIR      = "spleen/5th_run_pseudo_tuned"
...
train_transform = A.Compose([
    ...
    A.Rotate(limit=10, border_mode=0, p=0.35),
    A.Affine(
...
RUN_CONFIG = {
    ...
    "COMMENT":"""pseudo-label tuned spleen run: stable 384 input, softer augmentation, lower lr, and safer grayscale-to-binary mask thresholding for mixed manual and pseudo masks
            """
}
...
class SpleenMaskDataset(Dataset):
    ...
    def _ensure_mask_binary(self, mask):
        ...
        if mask.max() > 1:
            return (mask > MANUAL_MASK_FG_THRESHOLD).astype(np.uint8)
...
def collect_labeled_pairs(images_root=IMAGES_ROOT, masks_root=LABEL_MASKS_ROOT):
    ...
    for img_path in images_root.rglob("*"):
        ...
        if key in mask_lookup:
            ...
            pairs.append((str(img_path), str(mask_lookup[key]), label_value))
        else:
...
def main():
    ...
    mask_paths = [m for _, m, _ in pairs]
    ...
    all_history = []
    fold_idx = 0
    for train_idx, val_idx in kf.split(pairs, labels):
        fold_idx += 1
        print(f"\n=== Starting fold {fold_idx}/{KFOLDS} ===")
        train_pairs = [(img, mask) for img, mask, _ in [pairs[i] for i in train_idx]]
        val_pairs = [(img, mask) for img, mask, _ in [pairs[i] for i in val_idx]]
