...
PATIENCE = 10
THRESH = 0.5 #cheange told by gpt , that too low
NUM_GPUS_TO_USE = 2
KFOLDS = 5
SEED = 42
...
RUN_CONFIG = {
    ...
    "COMMENT":"""changes image size , threshold to .5, encoder 3 
            """
}
...
def train_and_evaluate_fold(model, train_loader, val_loader, fold_index, epochs=EPOCHS, save_dir=RESULTS_DIR):
    ...
    for epoch in range(1, epochs+1):
        ...
        print(f"Fold {fold_index} Epoch {epoch}: val_iou={avg_metrics['iou']:.4f},train_acc={train_accuracy:.4f}, val_acc={avg_metrics['accuracy']:.4f}, train_loss={train_loss:.4f}, val_loss={val_loss:.4f}")
